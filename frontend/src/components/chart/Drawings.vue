...
<template>
  ...
  export default {
    ...
    emits: ['chart-click', 'edit-line'], // Добавлено для устранения предупреждения о лишних слушателях
    setup() {
    ...
    methods: {
      ...
      handleChartClick({ price, time, point }) {
        ...
        if (this.chartStore.drawingTool === 'line') {
          ...
          let raySeries;
          try {
            raySeries = this.chart.addSeries(LineSeries, {
              color: '#FFD700',
              lineWidth: 1,
              lineStyle: LineStyle.Dashed,
          ...
          if (!this.trendlineStart) {
            ...
            let trendlineSeries;
            try {
              trendlineSeries = this.chart.addSeries(LineSeries, {
                color: '#FFD700',
                lineWidth: 1,
      ...
      loadDrawings(drawings) {
        if (!this.chart) {
          console.warn('График не инициализирован, пропуск загрузки линий'); // Добавлена проверка
          return;
        }
        ...
        drawings.forEach(({ id, drawing_type, symbol, data }) => {
          ...
          try {
            if (drawing_type === 'drawing.line') {
              ...
              let raySeries = this.chart.addSeries(LineSeries, {
                color: parsedData.color,
                lineWidth: parsedData.line_width,
                lineStyle: LineStyle.Dashed,
                priceLineVisible: false,
              ...
              let trendlineSeries = this.chart.addSeries(LineSeries, {
                color: parsedData.color,
                lineWidth: parsedData.line_width,
                lineStyle: LineStyle.Solid,
                priceLineVisible: false,
      ...
      updateLineProperties() {
        ...
        if (line.drawing_type === 'drawing.line') {
          ...
          try {
            updatedLine = this.chart.addSeries(LineSeries, {
              color: this.selectedLine.data.color,
              lineWidth: this.selectedLine.data.line_width,
              lineStyle: LineStyle.Dashed,
              priceLineVisible: false,
